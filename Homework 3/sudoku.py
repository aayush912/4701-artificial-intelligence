...
def solve(config, domains):
    if solution(config, domains):
        return domains
    ans = None
    key = min([domain for domain in config.dict if domain not in domains], key=lambda Key: len(config.dict[Key])) # MRV Heuristic
        ...
        if consistent == True:
            ...
            for neighbor in neighbors(newconfig, key):
                if neighbor not in domains:
                    newconfig.dict[neighbor] = newconfig.dict[neighbor].replace(value, '')
